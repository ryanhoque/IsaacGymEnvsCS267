...
class A2CBase:
    def __init__(self, base_name, config):
        ...
        print('Env info:')
        print(self.env_info)
        print(self.vec_env.env.reward_calculate_counter)
    ...
    def env_step(self, actions):
        ...
        #     print("env step took", (time.time() - time_before)*1000000)
...
class ContinuousA2CBase(A2CBase):
    ...
    def train_epoch(self, i=0):
        super().train_epoch()

        self.set_eval()
        play_time_start = time.time()
        #if i == 3 and PROF_SECTION == 'play':
        #    profiler.start()
        with torch.no_grad():
            if self.is_rnn:
                batch_dict = self.play_steps_rnn()
        ...
        play_time_end = time.time()
        update_time_start = time.time()
        #if i == 3 and PROF_SECTION == 'update':
        #    profiler.start()
        after_1_epoch_time = None
        rnn_masks = batch_dict.get('rnn_masks', None)
        ...
        update_time = update_time_end - update_time_start
        total_time = update_time_end - play_time_start
        print("Play time: ", play_time, "Update time:", update_time)
    ...
    def train(self):
        ...
        while True:
            epoch_num = self.update_epoch()
            step_time, play_time, update_time, sum_time, a_losses, c_losses, b_losses, entropies, kls, last_lr, lr_mul = self.train_epoch(epoch_ctr)
            total_time += sum_time
            frame = self.frame
            epoch_ctr += 1
            ...
            if self.rank == 0:
                ...
                if epoch_num > self.max_epochs:
                    ...
                    print('MAX EPOCHS NUM!')
                    print('reward calculate counter', self.vec_env.env.reward_calculate_counter)
                    total_time_since_1_epoch = time.time() - after_1_epoch_time
                    print("total time since 1 epoch to run", total_time_since_1_epoch)
                    self.vec_env.env.TIMES = self.vec_env.env.TIMES[100:]
                    print("average & stdev & total & count reward computation time", np.mean(self.vec_env.env.TIMES)*1e6, np.std(self.vec_env.env.TIMES)*1e6, np.sum(self.vec_env.env.TIMES)*1e6, len(self.vec_env.env.TIMES))
                    should_exit = True
            ...
            after_1_epoch_time = time.time() if after_1_epoch_time is None else after_1_epoch_time
            # print(after_1_epoch_time)
